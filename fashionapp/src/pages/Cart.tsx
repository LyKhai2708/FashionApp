...
export default function Cart() {
    ...
    const handleUpdateQuantity = async (itemId: number, newQuantity: number, stockQuantity: number) => {
        ...
        if (newQuantity > stockQuantity) {
            message.warning(`Only ${stockQuantity} items left in stock`);
            return;
        }
            ...
            <div
                ...
                <div className="mt-10 w-full grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="flex item-end justify-center flex-col">
                        {isEmpty ? (
                            <div className="w-full flex flex-col items-center justify-center text-center py-10">
                                ...
                                <div className="mt-6 text-xl font-semibold text-gray-900">Your cart is empty</div>
                                <div className="mt-1 text-gray-600">Let's <span className="text-gray-800 font-medium">add</span> some products to your cart!</div>
                                <a href="/" className="mt-6 inline-flex items-center justify-center px-8 py-3 rounded-full border border-gray-400 text-gray-800 hover:bg-gray-100 transition cursor-pointer">Shop now</a>
                            </div>
                        ) : (
                            <>
                                <div className="flex items-center gap-2 mb-4">
                                    <h2 className="font-semibold text-2xl">YOUR CART</h2>
                                    <span>(You have <span className="font-bold">{items.length}</span> products)</span>
                                </div>
                                <div className="mt-4">
                                                        ...
                                                        <div className="flex flex-col gap-1">
                                                            ...
                                                            {item.variant.stock_quantity < 10 && (
                                                                <span className="text-orange-500 text-xs font-medium">
                                                                    Only {item.variant.stock_quantity} items left
                                                                </span>
                                                            )}
                                                        </div>
                                                    </div>
                    ...
                    <div className="bg-gray-50 h-fit p-6 rounded-md shadow-lg">
                        ...
                        <Link to="/" className="block text-center mt-3 text-gray-600 hover:underline">Continue shopping</Link>
                    </div>
                </div>

                <div className="flex items-center gap-2 mb-4 mt-10">
                    <h2 className="font-semibold text-2xl">RECENTLY VIEWED PRODUCTS</h2>
                </div>
